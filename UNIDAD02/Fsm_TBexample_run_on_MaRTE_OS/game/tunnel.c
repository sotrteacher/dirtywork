#define LMC_11000
#define LMC_12000
#define LMC_13000  /*Transition from initial to playing directly*/
//#define LMC_14000
/*************************************************************************
...
#endif /*MINE2*/
    QFsm *bombs[GAME_MINES_MAX];        /** active bombs */
    QFsm *mine3_pool[GAME_MINES_MAX];   /** Pool for Bomb4 */
...
static void Tunnel_dispatchToAllBombs(Tunnel *me, QEvt const *e);
...
#endif /*MINE2*/
      me->mine3_pool[n] = Bomb4_ctor(n,7);/* idem Bomb4 in the pool*/
#ifdef MINE1
      me->mines[n] = (QHsm *)0;                /* mine 'n' is unused */
#endif /*MINE1*/
      me->bombs[n] = (QFsm *)0;                /* bomb 'n' is unused */
    }
...
QState Tunnel_initial(Tunnel *me, QEvt const *e) {
...
#endif /*MINE2*/
      QFsm_init(me->mine3_pool[n],(QEvt *)0); /* initial tran. for Bomb4 */
    }
    ...
    QS_SIG_DICTIONARY(BOMB_SIG,           &l_tunnel);
#ifndef LMC_13000
    return Q_TRAN(&Tunnel_demo);
#else
    //while(1){printf("Before return Q_TRAN(&Tunnel_playing);\n");}
...
#ifdef LMC_12000
    static int count_t_d=0;
    count_t_d++;
    printf("Tunnel_demo(), %d\n",count_t_d);
#endif /*LMC_12000*/
    switch (e->sig) {
        case Q_ENTRY_SIG: {
...
#ifdef LMC_14000
    static int b = 0;
    while(b<5) {printf("Tunnel_playing()\n");b++;}
#endif /*LMC_14000*/
        ...
        //case BOMB_SIG: {
        //    BSP_display(((TickEvt const *)e)->fine_time);
        //    return Q_HANDLED();
        //}
        case UP_SIG:  /*2020.12.11*/
        case DOWN_SIG: 
        case ARM_SIG: {
            Tunnel_dispatchToAllBombs(me,e);
            return Q_HANDLED();
        }
        case GAME_OVER_SIG: {
...
void Tunnel_plantMine(Tunnel *me) {
    ...
    for (m = 0; m < Q_DIM(me->bombs); ++m) {/* look for disabled bombs */
        if (me->bombs[m] == (QFsm *)0) {
            break;
        }
    }
    if (m < Q_DIM(me->bombs)) {     /* a disabled Bomb4 found? */
      ObjectPosEvt etdttm; /* event to dispatch to the bomb */
      me->bombs[m] = me->mine3_pool[m];
      etdttm.super.sig = MINE_PLANT_SIG;
      QFsm_dispatch(me->bombs[m],(QEvt *)&etdttm); /* direct dispatch */
    }
}/*end Tunnel_plantMine()*/
/*....................................................................*/
...
#endif /*MINE1*/
      if (me->bombs[n] != (QFsm *)0) {         /* is the bomb used? */
        QFsm_dispatch(me->bombs[n],e);
      }
    }
    (void)me;                           /* avoid the compiler warning */
}/*Tunnel_dispatchToAllMines()*/
/*....................................................................*/
void Tunnel_dispatchToAllBombs(Tunnel *me, QEvt const *e) {
  uint8_t n;
  for (n = 0; n < GAME_MINES_MAX; ++n) {
    if (me->bombs[n] != (QFsm *)0) {           /* is the bomb used? */
      QFsm_dispatch(me->bombs[n],e);
