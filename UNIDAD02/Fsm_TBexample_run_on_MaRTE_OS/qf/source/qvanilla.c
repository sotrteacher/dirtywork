...
/**
* \description
* QF_run_event_loop() executes the payload of each iteration of the kernel
* loop, and it is intended to be executed in a multi-threaded environment
* as a replacement for QF_run().
*
* \returns QF_run_event_loop() typically does not return in embedded
* applications.
* However, when QP runs on top of an operating system,  QF_run_event_loop()
* might return and in this case the return represents the error code (0 for
* success). Typically the value returned from QF_run_event_loop() is
* subsequently passed on as return from main().
*/
int_t QF_run_event_loop(void) {
    /* Ejecutar sólo una iteración del loop de eventos */
    /* the combined event-loop and background-loop of the "Vanilla" kernel */
    QEvt const *e;
    QActive *a;
    uint_fast8_t p;
