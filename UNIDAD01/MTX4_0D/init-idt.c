...
PRIVATE IDT_DESCRIPTOR idt1[256] ;
...
ISR GetISR(int int_numb)
	{
#ifdef SOTR_2021_A
  __asm__ __volatile__ ("sidt %0" : : "m" (idt1)) ;
	IDT_DESCRIPTOR *p = &idt1[int_numb] ;
PutString("\r\nIDTR:\r\n");
//PutUnsigned(p->offset_16,16,0);
//PutString("\r\np->offset_0\r\n");
//PutUnsigned(p->offset_0,16,0);
PutUnsigned((((DWORD32) p->offset_16) << 16) + p->offset_0,16,0);
  PutString("\r\nSO FAR SO GOOD 1\r\n");
#else
	IDT_DESCRIPTOR *p = &idt[int_numb] ;
#endif
	DWORD32 offset ;
//  PutString("SO FAR SO GOOD 2\r\n");
