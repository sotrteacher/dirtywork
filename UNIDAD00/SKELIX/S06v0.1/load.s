...
# setup up timer 8253 chip.
	...
	#movl $119300, %eax        # timer frequency 10 HZ 
	movl $0x40, %edx
	outb %al, %dx
	movb %ah, %al
	outb %al, %dx
...
move_to_user_mode:	# Move to user mode (task 0)
	...
	#pushl $0x17
     #else 
	pushl $0x17             # 00000001  0111 = 000000010 1 11, index = 2, T=1 (LDT), DPL=3
     #endif
	pushl $stack0_ptr
	pushfl
	pushl $0x0f
...
# originally placed at timer.c
# Called from subroutine timer_interrupt
#
my_do_timer:
	pushl	%ebp
	movl	%esp, %ebp
...
#endif
	pushl	$24                     ## 
	pushl	$71                     # set_cursor(71,24);
	call	set_cursor              #
	addl	$8, %esp                ##
#if 0 //LMC 2021.11.02	
	#movl	timer_ticks_Pt, %ecx
#else	
	movl	timer_ticks, %ecx
	addl	$1, %ecx
	movl	%ecx, timer_ticks
#endif
	pushl	%ecx					##
	movl	$percent_x_string, %ecx # kprintf(KPL_DUMP,"%x",timer_ticks);
	pushl	%ecx					# 
	pushl	$0						# KPL_DUMP is 0
	call	kprintf                 #
	addl	$12, %esp               ##  
	pushl	%ebx
	pushl	%eax
	call	set_cursor
	addl	$8, %esp		# The calling code also needs to pop off all the parameters
							# it pushed onto the stack in order to get the stack pointer 
							# back where it was (you can simply add 4*number of parameters)
							# to %esp using the addl instruction, if you don't need the 
							# values of the parameters anymore). 
							# REF. ProgrammingGroundUp-1-0-booksize.pdf, 
							# Chapter 4 All About Functions, apge 58.
	popl	%ecx
	popl	%ebx
	popl	%eax
	movl	%ebp, %esp
...
#	.fill 128,4,0
#else
	.fill 512,4,0			# sizeof TASK0_STACK is 512
	#.fill 1024,4,0			# sizeof TASK0_STACK is 1024
#endif
stack0_krn_ptr:
	.long 0
...
#	.fill 128,4,0
#else
	.fill 512,4,0			# sizeof TASK1_STACK is 512
	#.fill 1024,4,0			# sizeof TASK1_STACK is 1024
#endif
stack1_krn_ptr:
	.long 0
